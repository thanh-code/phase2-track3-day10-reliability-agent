    ...
    for breaker in gateway.breakers.values():
        ...
        for entry in breaker.transition_log:
            if entry["to"] == "open":
                ...
                open_ts = None  # Reset to track next open→closed pair
...
def run_scenario(config: LabConfig, queries: list[str], scenario: ScenarioConfig) -> RunMetrics:
    ...
    6. Return metrics
    """
    overrides = scenario.provider_overrides if scenario.provider_overrides else None
    ...
    for _ in range(config.load_test.requests):
        ...
        if result.cache_hit:
            ...
            # Cache hit counts as successful
